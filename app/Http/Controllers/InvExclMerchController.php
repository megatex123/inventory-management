...
class InvExclMerchController extends Controller
{
    public function index(Request $request)
    {
        $query = InvExclMerch::with(['masterSku']);
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('item_name', 'LIKE', "%{$search}%")
                    ...
                    ->orWhere('inv_excl_merch_id', 'LIKE', "%{$search}%");
            });
        }
    ...
    {
        $item = InvExclMerch::with(['masterSku'])->find($id);
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $nextId = InvExclMerch::count() + 1;
            $code = 'IE-QVMR-' . str_pad($nextId, 4, '0', STR_PAD_LEFT);

            $item = InvExclMerch::create([
                'inv_excl_merch_id' => $code,
    ...
    public function statistics()
    {
        return response()->json([
            'success' => true,
            'data' => [
                'total_items' => InvExclMerch::count(),
                'total_stock' => (int) InvExclMerch::sum('current_stock'),
                'low_stock_count' => InvExclMerch::whereColumn('current_stock', '<', 'to_restock')->count(),
            ],
        ]);
    }
}

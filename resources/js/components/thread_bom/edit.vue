        ...
        <form v-else @submit.prevent="submit">
          <div class="row">
            <div class="col-md-4">
              <div class="form-group">
                ...
                <input type="text" v-model="form.psu_brand" class="form-control" required maxlength="100">
              </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
          ...
          <div class="form-actions mt-4 pt-3 border-top">
            <button type="submit" class="btn btn-primary" :disabled="loading">
              ...
              <i v-else class="fas fa-save mr-2"></i> Update BOM
            </button>
          </div>
        </form>
      </div>
...
export default {
  data() {
    return {
      masterSkus: [],
      form: { psu_brand: '', cable_type: '', colour_variant: '', is_default: false, lines: [{ sku_code: '', item_name: '', qty_per_cable: 1, unit_cost: '' }] },
      loading: false,
      loadingData: true,
      errors: []
    };
  ...
  methods: {
    async fetchEditData() {
      ...
      try {
        const res = await axios.get(`/api/thread-bom/${this.$route.params.id}/edit`);
        const data = res.data.data;
        this.masterSkus = data.master_skus || [];
        const record = data.thread_bom_header;
        this.form = {
          psu_brand: record.psu_brand,
          cable_type: record.cable_type,
          colour_variant: record.colour_variant,
          is_default: !!record.is_default,
          lines: (record.lines || []).map(l => ({
            sku_code: l.sku_code,
            item_name: l.item_name,
            qty_per_cable: l.qty_per_cable,
            unit_cost: l.unit_cost
          }))
        };
        if (this.form.lines.length === 0) {
          this.form.lines = [{ sku_code: '', item_name: '', qty_per_cable: 1, unit_cost: '' }];
        }
      } catch (error) {
        console.error('Error fetching BOM:', error);
        Swal.fire('Error!', 'Failed to load BOM', 'error').then(() => this.$router.push('/thread-bom'));
      } finally {
    ...
    submit() {
      ...
      axios.put(`/api/thread-bom/${this.$route.params.id}`, this.form)
        .then(() => {
          Swal.fire({ title: 'Success!', text: 'BOM updated successfully', icon: 'success', timer: 1500, showConfirmButton: false })
        ...
        .catch(error => {
          if (error.response && error.response.status === 422) {
            ...
            this.errors.push(error.response?.data?.message || 'Failed to update BOM');
          }

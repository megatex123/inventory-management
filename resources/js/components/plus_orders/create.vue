    ...
    <div class="card shadow-sm form-card">
      <div class="card-header bg-primary text-white">
        <div class="d-flex justify-content-between align-items-center">
          <h4 class="mb-0"><i class="fas fa-plus-circle mr-2"></i>Add Plus Order</h4>
        ...
        <form @submit.prevent="submit">
          ...
          <div class="form-actions mt-4 pt-3 border-top">
            <button type="submit" class="btn btn-primary" :disabled="loading">
              ...
              <i v-else class="fas fa-save mr-2"></i> Create Plus Order
            </button>
          </div>
        </form>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetchPlusServices() {
      try {
        const res = await axios.get('/api/plus-services', { params: { per_page: 1000 } });
        this.plusServices = res.data.data || [];
      } catch (error) {
    ...
    submit() {
      ...
      axios.post('/api/plus-orders', this.form)
        .then(() => {
          Swal.fire({ title: 'Success!', text: 'Plus order created successfully', icon: 'success', timer: 1500, showConfirmButton: false })
        ...
        .catch(error => {
          if (error.response && error.response.status === 422) {
            ...
            this.errors.push(error.response?.data?.message || 'Failed to create plus order');
          }

    ...
    <div class="card shadow-sm form-card">
      <div class="card-header bg-warning text-dark">
        <div class="d-flex justify-content-between align-items-center">
          <h4 class="mb-0"><i class="fas fa-edit mr-2"></i>Edit QuiviMerch Inventory Item</h4>
          <router-link to="/inv-merch" class="btn btn-light btn-sm"><i class="fas fa-arrow-left mr-1"></i> Back to List</router-link>
        </div>
      </div>
      <div class="card-body">
...
export default {
  ...
  methods: {
    async fetchEditData() {
      ...
      try {
        const res = await axios.get(`/api/inv-merch/${this.$route.params.id}/edit`);
        const data = res.data.data;
        this.masterSkus = data.master_skus || [];
        const record = data.inv_merch;
        this.form = {
        ...
        Swal.fire('Error!', 'Failed to load inventory item', 'error').then(() => this.$router.push('/inv-merch'));
      } finally {

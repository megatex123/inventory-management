<template>
  <div class="container my-5">
    <div v-if="loading" class="text-center">
      ...
      <p>Loading UAT meeting...</p>
    </div>
    <div v-else class="card shadow-sm form-card">
      <div class="card-body">
        <div class="text-center mb-4">
          <h2>{{ isEditMode ? 'Edit' : 'Create' }} UAT Meeting</h2>
        </div>
        <form @submit.prevent="submitUatMeetings">
          ...
          <div class="form-group mt-4">
            <button class="btn btn-success" :disabled="loading">
              {{ loading ? 'Saving...' : (isEditMode ? 'Update' : 'Save') }} UAT Meeting
            </button>
            ...
            <button type="button" class="btn btn-outline-danger ml-2" @click="deleteUatMeeting" v-if="isEditMode">
              Delete
            </button>
          </div>
        </form>
...
export default {
  ...
  methods: {
    ...
    fetchUatMeeting() {
    this.loading = true;
    console.log('Fetching UAT meeting with ID:', this.id);

    axios.get(`/api/uat-meeting/${this.id}`)
        .then(res => {
        ...
        this.showError('Failed to load UAT meeting');
        })
        .finally(() => {
        this.loading = false;
        });
    ...
    const url = this.isEditMode
        ? `/api/uat-meeting/${this.id}`
        : '/api/uat-meeting';

    axios[method](url, formData)
        .then(() => {
        const message = this.isEditMode
            ? 'UAT meeting updated successfully!'
            : 'UAT meeting created successfully!';
        alert(message);
        ...
        if (error.response && error.response.status === 422) {
            ...
            console.error('Error saving UAT meeting:', error);
            this.showError('Failed to save UAT meeting');
        }
        })
        .finally(() => {
        this.loading = false;
    ...
    deleteUatMeeting() {
      if (confirm('Are you sure you want to delete this UAT meeting?')) {
        this.loading = true;
        axios.delete(`/api/uat-meeting/${this.id}`)
          .then(() => {
            alert('UAT meeting deleted successfully!');
            this.$router.push('/meeting');
          })
          .catch(error => {
            console.error('Error deleting UAT meeting:', error);
            this.showError('Failed to delete UAT meeting');
          })
          .finally(() => {
            this.loading = false;
          });

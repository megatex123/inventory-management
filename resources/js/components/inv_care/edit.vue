    ...
    <div class="card shadow-sm form-card">
      <div class="card-header bg-warning text-white">
        <div class="d-flex justify-content-between align-items-center">
          <h4 class="mb-0"><i class="fas fa-edit mr-2"></i>Edit QuiviCare Inventory Item</h4>
...
export default {
  ...
  methods: {
    ...
    async fetchEditData() {
      ...
      try {
        const res = await axios.get(`/api/inv-care/${this.$route.params.id}/edit`);
        const data = res.data.data;
        this.masterSkus = data.master_skus || [];
        const record = data.inv_care;
        this.form = {
        ...
        Swal.fire('Error!', 'Failed to load inventory item', 'error').then(() => this.$router.push('/inv-care'));
      } finally {
    ...
    submit() {
      ...
      axios.put(`/api/inv-care/${this.$route.params.id}`, this.form)
        .then(() => {

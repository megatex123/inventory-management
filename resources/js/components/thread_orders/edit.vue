    ...
    <div class="card shadow-sm form-card">
      <div class="card-header bg-warning text-dark">
        <div class="d-flex justify-content-between align-items-center">
          <h4 class="mb-0"><i class="fas fa-edit mr-2"></i>Edit Thread Order</h4>
        ...
        <form v-else @submit.prevent="submit">
          ...
          <div class="form-actions mt-4 pt-3 border-top">
            <button type="submit" class="btn btn-primary" :disabled="loading">
              ...
              <i v-else class="fas fa-save mr-2"></i> Update Thread Order
            </button>
          </div>
        </form>
      </div>
...
export default {
  data() {
    return {
      ...
      form: { customer_id: '', status: 'pending', items: [] },
      loading: false,
      loadingData: true,
      errors: []
    };
  ...
  methods: {
    ...
    async fetchEditData() {
      ...
      try {
        const res = await axios.get(`/api/thread-orders/${this.$route.params.id}/edit`);
        const data = res.data.data;
        if (data.psu_brands && data.psu_brands.length) this.psuBrands = data.psu_brands;
        const record = data.thread_order;
        this.form = {
          customer_id: record.customer_id,
          status: record.status,
          items: (record.items || []).map(i => ({
            psu_brand: i.psu_brand,
            cable_type: i.cable_type,
            colour_variant: i.colour_variant || '',
            qty: i.qty,
            wire_length_cm: i.wire_length_cm,
            sleeve_length_cm: i.sleeve_length_cm,
            unit_price: i.unit_price,
            components: i.resolved_components || [],
            resolving: false,
            lineTotalDisplay: (parseFloat(i.unit_price || 0) * (parseInt(i.qty) || 0)).toFixed(2)
          }))
        };
        if (this.form.items.length === 0) {
          this.form.items = [{ psu_brand: '', cable_type: '', colour_variant: '', qty: 1, wire_length_cm: '', sleeve_length_cm: '', unit_price: '', components: [], resolving: false, lineTotalDisplay: '0.00' }];
        }
      } catch (error) {
        console.error('Error fetching thread order:', error);
        Swal.fire('Error!', 'Failed to load thread order', 'error').then(() => this.$router.push('/thread-orders'));
      } finally {
    ...
    addLine() {
      this.form.items.push({ psu_brand: '', cable_type: '', colour_variant: '', qty: 1, wire_length_cm: '', sleeve_length_cm: '', unit_price: '', components: [], resolving: false, lineTotalDisplay: '0.00' });
    },
    removeLine(idx) {
    ...
    submit() {
      ...
      axios.put(`/api/thread-orders/${this.$route.params.id}`, payload)
        .then(() => {
          Swal.fire({ title: 'Success!', text: 'Thread order updated successfully', icon: 'success', timer: 1500, showConfirmButton: false })
        ...
        .catch(error => {
          if (error.response && error.response.status === 422) {
            ...
            this.errors.push(error.response?.data?.message || 'Failed to update thread order');
          }

...
export default {
  ...
  mounted() {
    ...
    axios.get(`/api/meetings/${this.id}`).then(res => {
        this.form.customer_id = res.data.customer.id;
        this.form.title = res.data.title;
        this.form.meeting_date = res.data.meeting_date;
        this.form.meeting_notes = res.data.meeting_notes;
        this.existingDocument = res.data.document; // 👈 HERE
    });
  },
  methods: {
    ...
    submitMeeting() {
      ...
      axios.post(`/api/meetings/${this.id}?_method=PUT`, formData)

      ...
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div>
          ...
          <button
            ...
            :disabled="!onsiteHandover || onsiteHandover.status === 'completed'"
            @click="markComplete"
          >
            <i class="fas fa-check-circle mr-1"></i>
            {{ onsiteHandover && onsiteHandover.status === 'completed' ? 'Completed' : 'Mark Complete' }}
          </button>
        </div>
      </div>
  ...
  methods: {
    ...
    markComplete() {
      axios.post(`${this.apiBase}/complete`)
        .then(res => {
          this.onsiteHandover = res.data.data;

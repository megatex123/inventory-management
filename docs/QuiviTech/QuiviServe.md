...
Created by `OrderController::updateserve()` on approval — see [[QuiviCraft]]. Order-list quick-launch buttons (`allorder.vue`) find-or-create the `ServeBek`/`ServeMps`/`ServePce` record via `GET /api/{serve-bek|serve-mps|serve-pce}/order/{orderId}` — PCE uses the same quick-launch pattern as BEK/MPS, not a different flow; `ServePceController::getByOrder()` is structurally identical to the BEK/MPS equivalents. Unlike `ServeMps::getByOrder()` (fixed to seed `date_start` from `serve_data.start_serve_date`), PCE's `getByOrder()` does NOT seed this field — same disconnect as `ServeBek`, confirmed live and left unfixed.

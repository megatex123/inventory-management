...
`auth`, `brand`, `care`, `care_data`, `care_warranty`, `category`, `craft`, `craft_inspection`, `customer`, `customer_progress`, `employee`, `expens`, `inv_care`, `inventory_movement`, `master_sku`, `meeting`, `meeting_details`, `order`, `pos`, `product`, `refunds`, `salary`, `serve`, `serve_bek`, `serve_data`, `serve_mps`, `serve_pce`, `stock`, `sub_category`, `suppliers`, `uat_meeting` (added 2026-07-20, redesigned 2026-08-10 — see below, no longer identical to `meeting_details`). (`product_warranty` removed 2026-07-20 — see [[Product-Warranty]]. `inv_excl_serve` removed 2026-08-02 — see [[API-Routes]].)
...
- **`resources/js/components/uat_meeting/`** redesigned 2026-08-10 from its original "identical to `meeting_details`" shape into a change-request-against-an-order UI — see [[API-Routes]] for the backing `UatMeetingController` contract change.
  - `create.vue`/`edit.vue` are now organized into 4 sections: **Header** (Meeting dropdown, "Require Meeting ID" dropdown sourced from the new `GET /api/meeting-details/all`, QV-BLDP ID dropdown, and an auto-generated, read-only UAT Meeting ID); **Changes Requested** (`budget_change`, `parts_changes`, `add_on_parts`, `parts_notes`, `case_size_change`, `overall_notes`); **Build Details** (`target_build_date`, `target_location`); **Service Changes** (`quivicare_change`/`quivithread_change` enum dropdowns — record-only, no side effects on QuiviCare/QuiviServe pricing or tier, unlike the order-level "Upgrade PCE?" toggle documented in [[API-Routes]]); **UAT Result** (`changes_required`/`new_proposal_required`/`customer_approval`/`follow_up_required`, with `changes_required`/`new_proposal_required`/`follow_up_required` offering a blank/`-` option distinct from an explicit "No", since they're nullable booleans).
  - `index.vue`'s table columns are UAT Meeting ID, Meeting/Customer (meeting's `meeting_id` plus the related customer's `full_name` as a `<small class="text-muted">` sub-line — matches the pattern used in `meeting/index.vue` and `meeting_details/index.vue`), QV-BLDP ID, Customer Approval (badge), Changes Required (3-state badge: `badge-info`/"Yes" when `true`, `badge-secondary`/"No" when `false`, `badge-light`/"-" when `null`/`undefined` — distinguishes "not yet recorded" from an explicit "No"), and Target Build Date. Filters are `customerApproval` and `changesRequired` (plus free-text `search`); sorting via `SortableTh`/`sortablePaginationMixin` is available on Target Build Date only, matching the backend's `sort_by` allow-list.
  - `UatMeetingController::index()` (the list endpoint only) eager-loads just `meeting.customer` — `requirementMeeting`/`order` are loaded on `show()`/`store()`/`update()` for `edit.vue`'s and the create/update response's use, but were trimmed from `index()` since the list page never reads them (wasted queries otherwise).
...
- **`resources/js/components/order/view.vue`'s `grandTotalAmount` computed property had a real, pre-existing bug fixed 2026-08-08** (commit `9c85741`, found via a real user report right after the Upgrade PCE fee bump shipped): its `carePrice` preferred `this.care.care_charge` — the generic `Care` tier lookup's static base rate (COR3/RI5E/VIS10N) — over the order's actual computed `order.care_data[0].price`, whenever `this.care` was truthy (which is unconditionally true, since `fetchOrderDetails()` always sets it). This meant the page's "GRAND TOTAL AMOUNT" row silently ignored the real per-order QuiviCare fee (the RMA-eligible-parts bracket calculation, and now the Upgrade PCE +RM69.90 bump), while the sibling `totalPayAmount` computed property ("TOTAL DEPOSIT AMOUNT" row) correctly read `order.care_data[0].price` — so the same page showed two different QuiviCare-inclusive totals for the same order. Fixed by making `carePrice` prefer `order.care_data[0].price` first in both computed properties, falling back to `this.care.care_charge` only if `care_data` is genuinely unavailable. A dead `this.careCharge` reference (a `data()` property never assigned anywhere in the file, always `0`) was also removed from this one branch as incidental cleanup — `totalPayAmount`'s own (equally dead) reference to it was left untouched, out of scope for this fix.

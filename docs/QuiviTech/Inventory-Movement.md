...
- **`MasterSku`** — the inventory/spare-parts SKU catalog (not customer-facing sellable products — that's `Products`, see [[Product-Catalog]]). This is what `InvMove` and `InvCare` key off.
...
- **`InvCare`** (spare/RMA parts) — a narrower, purpose-specific stock pool consumed by [[QuiviCare]]. **`InvMerch`** ([[QuiviMerch]]) and **`InvThread`** ([[QuiviThread]]) follow the exact same pattern, added 2026-07-15. All three key off `master_sku.sku_code`, but **none are drawn from or reconciled against `InvMove`** — each is an independent stock counter, not derived from the general ledger, and none auto-decrement when an order/claim consumes stock — every one is only ever adjusted via its own CRUD `update()`. `InvMerch` previously had a separate `InvExclMerch` twin (general vs. exclusive QuiviMerch stock, two tables/controllers/pages) — merged into one `inv_merch` table with an `is_exclusive` flag on 2026-08-01; the distinct `InvExclMerch` model/controller/routes/frontend no longer exist. **`InvExclServe`** (service-exclusive consumables, formerly consumed by [[QuiviServe]]) was **removed entirely on 2026-08-02** (0 live rows, per the project owner's explicit request) — see [[API-Routes]] and [[Domain-Models]] for the removal detail.
...
- [[QuiviServe]] — formerly `InvExclServe` consumer (removed 2026-08-02)
